...
/* Exception and Interrupt Handlers with Specific Attributes */
...
/* Interrupt Vector Table Array Definition */
/* The precise placement and order of these handlers is detailed in the Reference Manual document. */
/* The zeros act as placeholders for the positions corresponding to interrupts not supported by our specific microcontroller variant. */
/* The table consists of memory addresses pointing to the start of ISR handlers, which are 32 bits in length for ARM Cortex-M microcontrollers. */
uint32_t vector_tbl[] __attribute__((section(".isr_vector_tbl"))) = {
    (uint32_t)&_estack,                       // The initial top of the stack in the memory
    ...
    (uint32_t)&PVD_IRQHandler,                // EXTI Line 16 interrupt / PVD through EXTI line detection interrupt
    ...
    (uint32_t)&ADC_IRQHandler,                // ADC1 global interrupts
    0,                                        // Reserved
    0,                                        // Reserved
    0,                                        // Reserved
    0,                                        // Reserved
...
/* Default Handler as a Universal Fallback */
/* This handler enters an infinite loop to prevent the program from proceeding into an undefined state following such an event. */
void Default_Handler(void) {
    while (1) {
    }
}
...
/* Reset Handler */
/* It prepares the system before executing the main application. */
void Reset_Handler(void) {
    // Calculate the sizes of the .data and .bss sections
    uint32_t data_mem_size = (uint32_t)&_edata - (uint32_t)&_sdata;  // It is used to copy initialized data from FLASH to SRAM.
    uint32_t bss_mem_size = (uint32_t)&_ebss - (uint32_t)&_sbss;     // It is used to zero out the .bss section in SRAM.

    // Initialize pointers to the source and destination of the .data section
    uint32_t *p_src_mem = (uint32_t *)&_etext;  // A source pointer to the address where initialized data is stored in flash memory
    uint32_t *p_dest_mem = (uint32_t *)&_sdata; // A destination pointer to the start of the .data section in SRAM

    // Copy .data section from FLASH to SRAM word by word
    for (uint32_t i = 0; i < data_mem_size; i++) {
        *p_dest_mem++ = *p_src_mem++;
    }

    // Initialize the .bss section to zero in SRAM word by word
    ...
    // Call the application's main function to begin the application execution
    main();
}
